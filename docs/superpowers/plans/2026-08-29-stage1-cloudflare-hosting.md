...
- **Stage 3 — the promote gate.** Spec §9. `promote-kids.bat`, the `DEV-VERIFIED.json` refusal, and the desktop shortcut. Until it exists, production is deployed by the temporary `npm run deploy:prod-preview` from Task 7, which is **not** a gate and must not be mistaken for one.
