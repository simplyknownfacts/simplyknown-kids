...
Status: INCOMPLETE full audit; baseline checkpoint saved. The current accepted product baseline is the Days notice repair `a23a0ea4ef49c2ddb79e62fbfb382fd98b1efde7`, independently reviewed in a separate checkout with no remaining P1/P2 finding.
...
4. Browser: initial spot checks used visible Codex in-app Chromium after Chrome creation timed out; the bounded cross-tier run used local Playwright Chromium with isolated synthetic profiles and no real family state. Desktop 1280×900 and touch-phone 390×844 are now partially exercised; Count Along, ABCs, Days and Math also have bounded 320×568 and 820×1180 probes. Physical devices, audio heard by a human, iOS sensor behavior and external services remain separate gaps.
