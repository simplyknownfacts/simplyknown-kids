...
## Flows
- **Add kid (real UI):** `index.html` `#addBtn` → `parent/settings.html?action=add` → pass the PIN gate → `#addForm` opens on `.add-step[data-step="1"]` with `#newName` focused. Fill `#newName` + `#newBirthday`, click `#addNext1`; choose `#newMascotPicker button[data-mascot]`, click `#addNext2`; choose `#newVoicePicker button[data-voice]`, then `#createChild`. The optional color and paid spoken-name offer live under `.add-more-options`; `#offerNameVoice` defaults off. The new child appears in `#settingsChildSelect` and `#profilesList .card`.
...
- **Settings layout:** narrow screens show exactly one `.settings-panel.active`; choose it with `#settingsSectionPicker`. Desktop uses `#sideNav .navitem[data-key="<key>"]`. `showPanel(key)` remains a harness fallback. Keys include overview, activities, features, voice, mascot, theme, coloring, youtube, children, yoto, sync, offline, and pin; panels use `#panel-<key>`. Select the edited child by profile id through `#settingsChildSelect`; there are no child pills.
- **Feature toggles:** panel `features`. Activities with no feature options are omitted. For each defined eligible option, locate its `#featuresTable` row by activity name, then `label.feat-label` by label text → `input[type=checkbox]`. A missing defined option is a failure; the checkbox auto-saves.
- **Activity show/hide:** panel `activities`. `#activitiesSection input.act-vis[data-pid][data-aid="<id>"]`. Auto-saves.
- **Voice:** panel `voice`. `#voiceSection .vcard[data-voice="<id>"]` (girl/boy/woman/man). Click → selects (class `sel`) + plays sample (stubbed). Assert `sel`.
- **Overview / theme / offline:** use section keys `overview`, `theme`, and `offline`. Their panels are `#panel-overview`, `#panel-theme`, and `#panel-offline`; on narrow screens select the key in `#settingsSectionPicker`, and on desktop click the matching `#sideNav .navitem`.
...
- `color-in`: visible `#colorFillCanvas[data-ready="1"]`; choose `[data-color="#FF5A67"]`, map source point `(200,150)` through the canvas box, then click. Assert source pixel RGB becomes `[255,90,103]`, the top ray `(200,45)` stays white, and the pupil/outline stay dark. `#undoFill` restores the prior color.
