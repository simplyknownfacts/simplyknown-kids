...
try{
  for(const item of CASES){
    ...
    const geometry=await page.evaluate(()=>{
      const stage=document.querySelector('#stage').getBoundingClientRect(),spots=[...document.querySelectorAll('.hiding-spot')].map((spot,index)=>{const box=spot.getBoundingClientRect(),x=box.left+box.width/2,y=box.top+box.height/2;return{index,left:box.left,top:box.top,right:box.right,bottom:box.bottom,width:box.width,height:box.height,reachable:document.elementFromPoint(x,y)?.closest('.hiding-spot')===spot,clue:[...spot.classList].filter(name=>name.startsWith('clue-'))}});return{renderer:document.querySelector('#stage').dataset.renderer,phase:document.querySelector('#stage').dataset.phase,peekPart:document.querySelector('#stage').dataset.peekPart,hint:document.querySelector('#hint').textContent,stage:{left:stage.left,top:stage.top,right:stage.right,bottom:stage.bottom},spots,minTarget:Math.min(...spots.map(spot=>Math.min(spot.width,spot.height))),allInside:spots.every(spot=>spot.left>=stage.left-1&&spot.top>=stage.top-1&&spot.right<=stage.right+1&&spot.bottom<=stage.bottom+1),allReachable:spots.every(spot=>spot.reachable)} });
    ...
    rows.push({...item,file,files,errors,geometry,pass:errors.length===0&&geometry.renderer==='webgl'&&geometry.phase==='seek'&&geometry.spots.length===(item.tier<=2?4:item.tier<=4?6:item.tier<=6?8:item.tier<=8?10:12)&&geometry.minTarget>=44&&geometry.allInside&&geometry.allReachable});
    await context.close();
  }
