...
const NOTICE_CASES = [
  ...
  { tier: 4, name: 'short-phone', width: 320, height: 568 },
  { tier: 10, name: 'tablet', width: 820, height: 1180 },
  { tier: 10, name: 'desktop', width: 1280, height: 900 },
];
const ANSWER_CASES = [
  ...[4, 6, 10].map(tier => ({ tier, name: 'short-phone', width: 320, height: 568 })),
...
function seed(tier) {
  const profile = {
    id: `clock-layout-t${tier}`,
    name: 'Clock Layout Test',
    birthday: '2020-01-01',
    color: '#4ECDC4',
    voice: 'girl',
    mascot: { id: 'dog' },
...
async function openClock(browser, port, entry) {
  ...
  await page.goto(`http://127.0.0.1:${port}/learning/clock.html`, { waitUntil: 'load' });
  await page.locator('.num-btn').first().waitFor();
  return { context, page };
}

test('Clock phone reward and short-phone answers stay unobscured', { timeout: 120000 }, async t => {
  ...
  try {
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('local Clock server did not start')), 12000);
      server.stdout.on('data', data => {
        if (String(data).includes('localhost:' + port)) { clearTimeout(timer); resolve(); }
      });
      server.once('error', error => { clearTimeout(timer); reject(error); });
      server.once('exit', () => { clearTimeout(timer); reject(new Error('local Clock server exited')); });
    });
    browser = await chromium.launch();

    for (const entry of NOTICE_CASES) {
      await t.test(`reward T${entry.tier} ${entry.name} ${entry.width}x${entry.height}`, async () => {
        const { context, page } = await openClock(browser, port, entry);
        try {
          await page.evaluate(() => {
            speakInstruction('What time is it?');
            vbCelebrate.show([{ id: 'clock-layout', type: 'milestone', tier: 'gold', title: 'Clock Star' }]);
          });
    ...
    for (const entry of ANSWER_CASES) {
      await t.test(`answers T${entry.tier} ${entry.name} ${entry.width}x${entry.height}`, async () => {
        ...
        try {
          ...
          assert.equal(geometry.answers.length, 4, 'Clock needs four answer choices');
          assert.ok(geometry.answers.every(answer => answer.height >= 64), 'Clock answer target shrank below 64px');
